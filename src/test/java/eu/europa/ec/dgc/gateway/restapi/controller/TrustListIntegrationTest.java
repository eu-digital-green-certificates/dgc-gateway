...
class TrustListIntegrationTest {
    ...
    void testTrustListDownloadPageableDefaultFallback() throws Exception {
        ...
        mockMvc.perform(get("/trustList?pagesize=5")
                ...
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(c -> assertTrustListLength(c, 5));

        KeyPairGenerator keyPairGenerator = KeyPairGenerator.getInstance("ec");
        for (int i = 0; i < 120; i++) {
            signerInformationTestHelper.createSignerInformationInDB(
                    "DE", "sigN" + i, CertificateTestUtils.generateCertificate(keyPairGenerator.generateKeyPair(),
                            "EU", "EUTest"), nowMinusOneHour.minusSeconds(i));
        }
        ...
        mockMvc.perform(get("/trustList")
                ...
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(c -> assertTrustListLength(c, 136));

        mockMvc.perform(get("/trustList?page=0")
                        .accept(MediaType.APPLICATION_JSON_VALUE)
                        .header(dgcConfigProperties.getCertAuth().getHeaderFields().getThumbprint(), authCertHash)
                        .header(dgcConfigProperties.getCertAuth().getHeaderFields().getDistinguishedName(), authCertSubject)
                )
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(c -> assertTrustListLength(c, 100));

        mockMvc.perform(get("/trustList?page=1")
                        .accept(MediaType.APPLICATION_JSON_VALUE)
                        .header(dgcConfigProperties.getCertAuth().getHeaderFields().getThumbprint(), authCertHash)
                        .header(dgcConfigProperties.getCertAuth().getHeaderFields().getDistinguishedName(), authCertSubject)
                )
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(c -> assertTrustListLength(c, 36));
    }
